...
|Метод передачи|Для передачи данных используется метод RAW POST  – поток данных передается в теле запроса, 
отправляемого клиентом на сервер. Данные передаются в формате XML (Extensible Markup Language (XML) 1.0).|
